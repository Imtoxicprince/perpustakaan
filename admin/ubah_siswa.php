...
<body>
    ...
    <?php 
    include "koneksi.php";
    $query_siswa=mysqli_query($koneksi,"select * from siswa where 
  ...
  "<?=$data_siswa['id_siswa']?>">
        Nama Siswa :
  <br><input type="text" name="nama_siswa" value=   "<?=$data_siswa['nama_siswa']?>" class="form-control"></br>
        Tanggal Lahir : 
  <br><input type="date" name="tanggal_lahir" value="<?=$data_siswa['tanggal_lahir']?>" class="form-control"></br>
        Jenis Kelamin : 
        <?php 
...
<option value="<?=$key_gender?>" <?=$selek?>><?=$val_gender?></option>
        ...
        Alamat : 
<br><textarea name="alamat" class="form-control" rows="4"><?=$data_siswa['alamat']?></textarea></br>
        Kelas :
        ...
        Username : 
<br><input type="text" name="username" value="<?=$data_siswa['username']?>" class="form-control"></br>
        Password : 
